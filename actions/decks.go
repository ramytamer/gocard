...
func (v DecksResource) Update(c buffalo.Context) error {
	// Get the DB connection from the context
	tx, ok := c.Value("tx").(*pop.Connection)
	if !ok {
		return fmt.Errorf("no transaction found")
	...
	verrs, err := tx.ValidateAndUpdate(deck)
	if err != nil {
		return err
	}
	...
	if verrs.HasAny() {
		return responder.Wants("html", func(c buffalo.Context) error {
			...
			return c.Render(http.StatusUnprocessableEntity, r.HTML("/decks/edit.plush.html"))
	...
	return responder.Wants("html", func(c buffalo.Context) error {
		...
		c.Flash().Add("success", T.Translate(c, "deck.updated.success"))

		// and redirect to the show page
		return c.Redirect(http.StatusSeeOther, "/decks/%v", deck.ID)
	}).Wants("json", func(c buffalo.Context) error {
		return c.Render(http.StatusOK, r.JSON(deck))
	}).Wants("xml", func(c buffalo.Context) error {
		return c.Render(http.StatusOK, r.XML(deck))
	}).Respond(c)
}
